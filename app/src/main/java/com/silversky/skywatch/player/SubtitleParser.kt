...
object SubtitleParser {
  ...
  fun parseSrt(content: String, logger: Logger? = null): List<SubtitleCue> {
    val cues = mutableListOf<SubtitleCue>()
    if (content.isBlank()) {
      logger?.debug("SRT content is empty or blank")
      return cues
    }
    ...
    while (i < lines.size) {
      ...
      if (line.contains(" --> ")) {
        ...
        if (timestamps != null) {
          ...
          logger?.error("Failed to parse timestamp line: '$line'")
        }
      }
      i++
    }
    ...
    if (cues.isEmpty()) {
      logger?.warn("No valid cues found in SRT content (total lines: ${lines.size})")
    } else {
      logger?.debug("Successfully parsed ${cues.size} cues")
    }
  ...
  private fun parseTimestampLine(line: String, logger: Logger?): Pair<Long, Long>? {
    ...
    val start = parseSingleTime(startPart)
    if (start == null) {
      logger?.error("Failed to parse start time: '$startPart' in line: '$line'")
      return null
    }

    val end = parseSingleTime(endPart)
    if (end == null) {
      logger?.error("Failed to parse end time: '$endPart' in line: '$line'")
      return null
    }
